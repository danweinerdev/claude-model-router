    ...
    with handle:
        for line in handle:
            ...
            if not escalated and message.get("role") == "user":
                ...
                if any(t.startswith("[router-escalation")
...
def registered_tier(agent_type, cwd):
    """The agent's declared tier from the registry, or None if unregistered.

    Never raises: a metrics record missing a tier is fine, a hook that throws
    on every subagent stop is not.
    """
    try:
        return router_config.tier_of(agent_type, cwd=cwd or os.getcwd())
    except Exception:
        return None
...
def main():
    ...
    record = {
        ...
        # Declared tier, so the report can separate "this agent cost more than
        # expected" from "this agent is registered at an expensive tier".
        # None for unregistered agents (another plugin's, or a project's own).
        "tier": registered_tier(agent_type, payload.get("cwd")),
        "model": model,
    ...
    path = os.environ.get("MODEL_ROUTER_METRICS_PATH") or os.path.expanduser(
        "~/.claude/model-router/metrics.jsonl"
    )
