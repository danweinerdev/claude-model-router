...
def test_report_aggregates_and_shows_savings(tmp_path):
    path = write_metrics(tmp_path, [
        {"agent_type": "model-router:scout", "model": "claude-haiku-4-5",
         "escalated": False, "input_tokens": 1_000_000, "output_tokens": 100_000,
         "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
        {"agent_type": "model-router:mechanic", "model": "claude-sonnet-5",
         "escalated": True, "input_tokens": 500_000, "output_tokens": 50_000,
         "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
    ])
    out = run_stats(path)
    assert "model-router:scout" in out and "model-router:mechanic" in out
...
def test_negative_saving_formats_with_leading_minus(tmp_path):
    ...
    path = write_metrics(tmp_path, [
        {"agent_type": "model-router:extractor", "model": "claude-sonnet-5",
...
def test_statusline_session_and_total(tmp_path):
    path = write_metrics(tmp_path, [
        {"session_id": "s1", "agent_type": "model-router:scout", "model": "claude-haiku-4-5",
         "escalated": False, "input_tokens": 1_000_000, "output_tokens": 0,
         "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
        {"session_id": "s2", "agent_type": "model-router:scout", "model": "claude-haiku-4-5",
         "escalated": False, "input_tokens": 1_000_000, "output_tokens": 0,
         "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0},
    ])
    out = subprocess.run(
    ...
    assert out == "router $9.00/$18.00 saved"
...
def test_losing_delegation_flagged_when_net_exceeds_baseline(tmp_path):
    ...
    assert "model-router:extractor" in out
...
def test_advice_silent_when_healthy(tmp_path):
    path = write_metrics(tmp_path, recent("model-router:scout", 10))
    assert run_advice(path) == ""


def test_advice_flags_high_escalation(tmp_path):
    runs = recent("model-router:scout", 6) + recent("model-router:scout", 4, escalated=True)
    path = write_metrics(tmp_path, runs)
    out = run_advice(path)
    assert "model-router:scout" in out and "one tier up" in out
...
def test_advice_flags_losing_route(tmp_path):
    ...
    path = write_metrics(tmp_path, recent(
        "model-router:extractor", 6, model="claude-sonnet-5",
...
def test_advice_flags_fat_handoffs(tmp_path):
    path = write_metrics(tmp_path, recent(
        "model-router:mechanic", 6, handoff_output_tokens=5_000))
    out = run_advice(path)
    assert "reply cap is not holding" in out


def test_advice_needs_minimum_runs(tmp_path):
    path = write_metrics(tmp_path, recent("model-router:scout", 4, escalated=True))
    assert run_advice(path) == ""


def test_advice_ignores_stale_records(tmp_path):
    path = write_metrics(tmp_path, recent(
        "model-router:scout", 10, escalated=True, ts=1.0))  # 1970: far outside window
    assert run_advice(path) == ""
