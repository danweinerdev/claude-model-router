#!/usr/bin/env python3
"""Model router metrics report: cost per agent tier, escalation rate, savings vs baseline.
...
def advice(records, now=None):
    ...
    for name in sorted(groups):
        ...
        if rate > ADVICE_ESCALATION_RATE:
            lines.append(
                f"router advice: {name} escalated {rate:.0%} of "
        ...
        if net >= base:
            lines.append(
                f"router advice: {name} net cost (${net:.2f}) meets or "
        ...
        if handoffs and sum(handoffs) / len(handoffs) > ADVICE_HANDOFF_TOKENS:
            ...
            lines.append(
                f"router advice: {name} replies average {avg:,.0f} tokens "
...
def main():
    ...
    parser.add_argument("--path", default=os.environ.get(
        "MODEL_ROUTER_METRICS_PATH",
        os.path.expanduser("~/.claude/model-router/metrics.jsonl")))
